...
- **Рушій розрахунку** — повний decimal без проміжних округлень, формули-літерали для кожного компонента, знімок параметрів на кожен розрахунок; профспілковий внесок утримується лише з членів профспілки.
- **Автоматична середньоденна** — кожен розрахунок зберігається покомпонентно, тож база для відпускних/лікарняних збирається з минулих місяців сама; налаштовувані правила «що входить у базу» + прев'ю перед проводкою.
- **Документи** — розрахунково-платіжна відомість (1:1 з робочим бланком бухгалтера, колонки A..BL) і розрахункові листи (по два на аркуш).
...
```bash
cd electron && ./build-win.sh   # → electron/release/PayrollCalc Setup 0.2.0.exe
```
